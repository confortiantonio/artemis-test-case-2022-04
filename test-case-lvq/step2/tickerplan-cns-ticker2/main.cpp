...
public:
    ...
    HelloWorldConsumer(const std::string& brokerURI, const std::string& address, int numMessages, bool useTopic = false, bool sessionTransacted = false, int waitMillis = 15 * 60 * 1000) :
        ...
        address(address) {
            ...
            << "brokerURI: "            << brokerURI << std::endl
            << "address: "              << address << std::endl
            << "numMessages: "          << numMessages << std::endl
            << "useTopic: "             << useTopic << std::endl
            << "sessionTransacted: "    << sessionTransacted << std::endl
            << "waitMillis: "           << waitMillis << std::endl
            << std::endl;
    }
    ...
    virtual void run() {
        ...
        try {
            ...
            if (this->sessionTransacted == true) {
                ...
                std::cout << "created session mode: SESSION_TRANSACTED " << std::endl;
            }
            else {
                //session = connection->createSession(Session::AUTO_ACKNOWLEDGE);
                session = connection->createSession();
                std::cout << "created session mode: AUTO_ACKNOWLEDGE no param " << std::endl;
            }

            // Create the destination (Topic or Queue)
            if (useTopic) {               
                destination = session->createTopic(address);
                std::cout << "created Topic destination" << std::endl;
            }
            else {
                destination = session->createQueue(address);
                std::cout << "created Queue destination" << std::endl;
            }
            ...
            /*while (true) {
                cms::MessageEnumeration* enumeration = queueBrowser->getEnumeration();
